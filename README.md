...
> the real control stack, and one of them picks and places a work-piece. There is no
> hardware interface and no automated link to anything physical, so nothing here is
> measured against reality yet. See [Status](#status). Saying so plainly is a project rule,
> not modesty — the previous iteration called itself a digital twin while containing no
> hardware interface at all.
...
|---|---|---|
| Charter, architecture, ADRs | **Written** — 31 decisions recorded, 8 layers specified | Each layer document carries a status marker; `./scripts/doctor` checks both |
...
| L0 facility model and generators | **Working** — 1 zone, 7 types, 14 assets, 5 stations; every derived artifact generated byte-identically | `./scripts/validate-model`; 204 tests in `tools/tests/` |
...
| L3 skills | **Partial** — all 6 have a server; `Detect` and `Transfer` have never been run against the simulator | `MoveTo`, `Pick`, `Place`, `Grasp` asserted in scenarios; `Detect`/`Transfer` by unit test only |
...
| Line orchestration from the topology | **Partial** — L4 builds the line from L0, owns handoff and recovery, and **no arm moves in any of its tests** | Unit tests against fake action servers; no end-to-end run |
| Grasping | **By friction, no simulation aid** — repeatable in position, **not in orientation** | [`docs/measurements/`](./docs/measurements/README.md), 2 published campaigns |
| Sensor-driven three-robot line | **Not working** — Phase 1.D. The belt and beam plugins run in Gazebo, but their topics are not bridged to ROS and nothing subscribes to them | — |
...
asserted about how a part sits in the jaws.
