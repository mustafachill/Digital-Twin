...
> **Today the rebuild is at L0.** The simulated cell comes up and its three arms move
> under the real control stack; there is no hardware interface and no automated link to
> anything physical, and the pick-and-place cycle does not complete. See
> [Status](#status). Saying so plainly is a project rule, not modesty — the previous
> iteration called itself a digital twin while containing no hardware interface at all.
...
|---|---|---|
| Charter, architecture, ADRs | **Written** — 28 decisions recorded, 8 layers specified | Each layer document carries a status marker |
...
| L0 facility model and generators | **Working** — 1 zone, 6 types, 14 assets, 5 stations; every derived artifact generated byte-identically | `./scripts/validate-model`; 132 host tests |
...
| Pick-and-place cycle | **Not working** — no work-piece has been grasped | `./scripts/scenario pick_and_place` fails |
| Sensor-driven three-robot line | Phase 1.D — conveyor and beam plugins are not wired in | — |
...
Two things worth knowing before you read a number out of this system. The cell layout is
**engineered, not surveyed**, so a measurement taken from the model does not transfer to
the building until the Phase 3 scan. And **scenarios are not reproducible**: a passing run
is evidence about that run only — see
...
- **Decisions are recorded before they are implemented.** Twenty-eight ADRs cover every
