...
/// A batch front end for the SHIPPED grasp arithmetic at `c38a42c`, and nothing else.
///
/// DERIVED FROM
/// `docs/measurements/2026-09-02-option-f-regions/harness/predicate_eval.cpp`, copied at
/// commit `3235cbc`. That directory is FROZEN (`docs/measurements/README.md`) and nothing
/// in it is edited from here. **Everything below this header block is byte-identical to
/// that file**; the header is rewritten because the header is the one part of it that
/// makes claims about a campaign, and the claims it made were about that campaign's
/// `criteria.md` and not about this one's.
///
/// WHAT THIS CAMPAIGN USES IT FOR, and it is a shorter list than the source file's:
///
///   * `position` -- `cite_skills::gripper_position_for`, which is what turns a stop
///     width in `criteria.md` section 5.1's grid into the `stop_upper_rad` the fixture is
///     given. Section 3 requires the value the rig DECLARES to come from the shipped
///     function through this program, never from the reference implementation.
///   * `width` -- `cite_skills::gripper_width_for`, which is I3's mapping and I7's
///     reference, and which carries `Grasp.Result.reached_width_m` back to a drive
///     position for `holding_S`'s input (section 2.1).
///   * `tolerance` -- `cite_skills::gripper_width_tolerance_m`, the shipped half of the
///     superseded condition section 2.2 solves in closed form for the floor.
///   * `holding`, `margin`, `resolve`, `maxwidth`, `minwidth`, `padoffset`, `travel`,
///     `parts` -- carried through unchanged, and used only for the section 2 cross-check
///     recorded in the block header. No campaign verdict comes from any of them.
///
...
/// program contributes is the conversion, the cross-check and the derived widths.
///
