#!/usr/bin/env bash
# Candidate C3, follow-up: the allocations BELOW the ones CPULIMIT_1 covered.
#
# A SEPARATE script rather than an edit to cpu_limit_trial.sh, because that one has
# produced data and the campaign convention freezes a harness once it has (../README.md).
# The follow-up is forced by CPULIMIT_1's own result: 12 CPUs -> 1.094, 4 -> 1.025,
# 2 -> 0.478, so the recorded 0.14 lies below every allocation measured and the only
# honest way to say where is to measure lower rather than to extrapolate.
#
# The original question this inherits: does a smaller CPU allocation reproduce 0.14?
#
...
apply 1;  sleep "$SEGMENT_S"
apply 0.5; sleep "$SEGMENT_S"
apply 12
