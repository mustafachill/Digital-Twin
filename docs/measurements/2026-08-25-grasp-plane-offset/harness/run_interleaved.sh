#!/usr/bin/env bash
# One INTERLEAVED block: bring the cell up headless, run N trials against it,
# alternating between the commanded grasp heights given, bring it down.
#
#   run_interleaved.sh <label> <trials> <h1,h2> [extra args for the harness...]
#
...
HEIGHT="${1:?comma-separated grasp heights in metres}"; shift
...
python3 "${HERE}/interleaved_block.py" \
    --heights "$HEIGHT" \
