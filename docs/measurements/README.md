...
| [`2026-09-04-waypoint-clearance/`](2026-09-04-waypoint-clearance/ANALYSIS.md) | Under hull geometry, how close does this cell pass to the objects around it, does the hull ever sit closer than the vendor mesh it was derived from, and does a body pass between two of the waypoints [ADR-0027](../adr/0027-pilz-planning-pipeline.md)'s gate checks? | **[`docs/open-work.md`](../open-work.md) #17 was refused, not cleared** — its own rule K-ii found no consecutive-waypoint interval anywhere carrying both a large enough step and a close enough bracketing distance **at the same time**, because *"the cell moves fast where it is far from everything and creeps where it is close"*. The campaign **predicted before its first trial that it would not test the question**, and it did not: its NOT OBSERVED carries that rule with it and **may not be read as a clearance of ADR-0027's residual**. **#49's margin half is measured and its refusal half is unanswerable through this door** — a trajectory the hull set refuses is never published, so refusals were counted and their geometry was not. Containment held on both admissible captures, no hull-only contact occurred — **registered in advance as evidencing nothing** — and real close approaches were found **on trajectories `ValidateSolution` accepted**, the closest a gripper finger against a conveyor. **Every distance verdict is published under a failed reproduction clause**: REPRO1 is NOT REPRODUCED on all three blocks, so none of them may be cited outside the directory as a settled measurement — and the diagnosis must travel with the verdict, because what differs between two `numpy` versions is an **argmax tie-break over a constant-zero array**, not a distance. `pick_and_place` is a **measured nothing**, cleanly captured and wholly dropped. **Half the sample was lost to arm descriptions reading back as zero characters — the third rig this has happened on**, nothing attributed. One prediction is **refuted**: a capture's tool-point step landed in the middle band nobody here had measured. 216 trajectories recorded, 108 admitted, nine scenario runs. **Chooses nothing** — no threshold, no geometry, no status. |
| [`2026-09-22-is-a-run-reproducible/`](2026-09-22-is-a-run-reproducible/ANALYSIS.md) | Is a run of this cell reproducible at all, and where does the irreproducibility enter? | **The cell does not reproduce, and this campaign is forbidden from saying where that comes from.** Two `pick_and_place` runs on `cell_b`, **one seed, one commit, minutes apart**, put the work-piece **0.763 mm** apart against a **1e-6 m** threshold and differed by **4.687 s** of wall clock — and **both passed**, both Pilz-only, which is the registered point demonstrated: **a scenario passing is not a statement that the run reproduces.** Everything else is refused. The control could not move the metric by more than the 1e-6 m it injected — the perturbation passed through at **unit gain** and missed the threshold by 1.17e-16 m — so **rule N fires and T1 and T2 are NOT ADMISSIBLE**: nobody may write that the physics engine was shown to reproduce, or that `gz sim --seed` was shown not to reach it. **Q4 is UNRESOLVED** and no localisation to the solver, the coupling or the planner may be read out of this directory. The threshold was **unsatisfiable by construction** on a translation-equivariant probe world, and was applied literally (V9) and recorded as wrong rather than corrected. It also writes down, for the first time and on **x86_64**, the symbol scan [ADR-0027](../adr/0027-pilz-planning-pipeline.md) rests on: that record is **confirmed**, and `libdart` carries **its own generator with its own seed setter**, which `gz sim --seed` does not reach. 14 trials registered, 14 valid, **0 instrument losses in every arm**. **Chooses nothing** — its §0 reserves every consequence to the project owner, and two runs are two runs and not a rate. |
...
- **Rates are rates over samples, not determinism claims.** Scenarios in this cell are not
  ...
  **Since 2026-09-22 that rests on a measurement of the cell's behaviour and not only on a
  reading of what the seed reaches:**
  [`2026-09-22-is-a-run-reproducible/`](2026-09-22-is-a-run-reproducible/ANALYSIS.md) ran one
  scenario twice under one seed at one commit and the work-piece ended up in two places.
  **Read it at the size of its evidence — two runs on one machine, which is not a rate** — and
  note that it is forbidden to say *why*: its own control did not clear, so where the
  irreproducibility enters is UNRESOLVED there and may not be resolved from there.
