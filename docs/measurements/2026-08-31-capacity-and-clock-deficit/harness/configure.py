...
flipped generator constant are never committed. `run_campaign.sh` reverts with
`git checkout` after every trial and again at the end.
...
def main() -> int:
    ...
    # `solo` is the committed state, so it is a no-op rather than a swap back: the
    # driver reverts with `git checkout` before every trial (criteria.md 5).
