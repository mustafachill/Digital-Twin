...
Written AFTER the data was collected. The rules it applies were not: they are read off
criteria.md, which was committed before the first trial, and they are applied literally
...
def validate(rec: dict, median_load: float | None) -> list[str]:
    ...
    if median_load is not None and rec.get("load_before"):
        load1 = rec["load_before"][0]
        if abs(load1 - median_load) > V6_LOAD_DRIFT * median_load:
            broken.append(f"V6:pre-trial load {load1:.2f} against median {median_load:.2f}")
    return broken
...
def main() -> int:
    ...
    for rec in records:
        parts = rec["label"].split("_")
        broken = validate(rec, median_load)
        sides = rec.get("sides") or {}
        row = dict(
    ...
    print(json.dumps({k: out[k] for k in ("n_records", "median_pre_trial_load1", "pooled")}, indent=1))
    return 0
