...
| [0053](0053-index-hardware-params-by-backend.md) | Index a backend's instance parameters by backend, and bind them into the generated description | Proposed — **nothing is implemented.** `hardware.params` reaches no generated artifact today, on any backend, and `tools/tests/test_hardware_params_unbound.py` pins that. The record's seven-clause promotion condition is all host-testable, because no physical arm exists; its decision 4 corrects the framing of [0040](0040-stop-a-joint-part-way-with-a-test-only-hardware-plugin.md)'s revisit item, which the implementing change owes that record as an amendment |
