...
| [0054](0054-key-the-hardware-opt-in-on-a-declared-fact.md) | Key the hardware opt-in on a fact the model declares, not on a backend's name | Proposed — **nothing is implemented.** A type may declare the vendor's physical `ros2_control` plugin under the backend id `sim`; the shipped validator reports zero findings, the generator emits a description naming that plugin, and `cite_bringup.plan.require_hardware_opt_in` compares `"sim" != "sim"` and permits the bring-up without ever consulting `CITE_ALLOW_HARDWARE`. `HardwareBackend` gains a required field stating whether the backend commands physical hardware, and four gates key on it instead of on the id. **This record must land before [0053](0053-index-hardware-params-by-backend.md)'s implementation merges**: that branch delivers the connection parameter the vendor component needs, which turns an accidentally fail-closed misconfiguration into a working, unguarded hardware path |
