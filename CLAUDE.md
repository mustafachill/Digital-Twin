...
- **The L0 model and its generators are built and proven** (Phase 1.B). `model/` describes
  **two cells** and `workspace/src/cite_generated/` holds everything derived from them:
  ...
  **It read `1 zone(s), 7 type(s), 15 asset(s), 5 station(s), across 15 file(s)` until that
  ...
  and not a re-audit**: ADR-0055 declares a second zone `cell_b`, a one-arm cell, and keeps
  ...
  both, and `validate-model` and `build` cover both.
  ...
  **Only one zone is up at a time** (ADR-0055 decision 3), because the three reserved
